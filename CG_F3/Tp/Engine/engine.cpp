...
void parseXML(TiXmlElement *elem, ContainerElement &parent)
{
	...
	else if(!strcmp(elem->Value(),"model"))
	{
		...
		float r = 1, g = 1, b = 1;
		if(elem->QueryFloatAttribute("R",&r) == TIXML_WRONG_TYPE)	std::cerr << "R tem tipo errado" << std::endl;

		if(elem->QueryFloatAttribute("G",&g) == TIXML_WRONG_TYPE)	std::cerr << "G tem tipo errado" << std::endl;

		if(elem->QueryFloatAttribute("B",&b) == TIXML_WRONG_TYPE)	std::cerr << "B tem tipo errado" << std::endl;
