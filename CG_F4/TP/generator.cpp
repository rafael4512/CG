...
void help(){
    ...
    cout << "|                                                                         |" << endl;
    cout << "| - torus [RADIUS] [WIDTH] [HEIGHT] [SLICE] [STACK]                       |" << endl;
    cout << "|      Creates a cone with the radius, width, height, number of slices    |" << endl;
    cout << "|      and stacks given.                                                  |" << endl;
    cout << "|                                                                         |" << endl;
...
int writeXMLFile(char *xmlfile, char *figurefile)
{
    ...
    //Vai a todos os irmãos verificar se o nome do ficheiro já está inserido no xmlfile
    while(aux)
    {
...
int main(int argc, char const *argv[])
{
    ...
    if(system("mkdir -p Engine/Figures/ ")){};

    std::ofstream file;
    file.open(argv[argc - 1], ios::trunc);

    if (strcmp("plane",grap) == 0 && argc == 4) drawPlane(file, atof(argv[2]));
    else if (strcmp("box",grap) == 0 && argc >= 6) {
        int divX = 1;
        int divZ = 1;
        int divY = 1;

        if (argc >= 7) divX = atoi(argv[5]);
        if (argc >= 8) divY = atoi(argv[6]);
        if (argc == 9) divZ = atoi(argv[7]);
        ...
        drawBox(file, atof(argv[2]), atof(argv[3]), atof(argv[4]), divX, divY, divZ, argc);
    }
    else if (strcmp("sphere",grap) == 0 && argc == 6) drawSphere(file, atof(argv[2]), atoi(argv[3])-1, atoi(argv[4])-1);
    else if (strcmp("cone",grap) == 0 && argc == 7) drawCone(file, atof(argv[2]), atof(argv[3]), atoi(argv[4]), atoi(argv[5]));
    else if (strcmp("torus",grap) == 0 && argc == 8) drawTorus(file, atof(argv[2]), atof(argv[3]), atof(argv[4]), atoi(argv[5]), atoi(argv[6]));
    else if (strcmp("patch",grap) == 0 && argc == 5)
    {
        std::string input(argv[3]);
        drawPatch(input, file, atoi(argv[2]));
	}
    else {
        printf("Error command not found!\n");
        return -1;
